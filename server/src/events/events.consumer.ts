...
export default class EventsConsumer {
   ...
   */
  async handleAuthLocalEmailVerificationSent(payload: AuthLocalEmailVerificationSentEvent): Promise<void> {
   ...
   * @returns {Promise<void>} A promise that resolves when the event has been handled.
   */
  async handleAuthLocalEmailVerificationVerified(payload: AuthLocalEmailVerificationVerifiedEvent): Promise<void> {
    await this.eventsService.createEvent(payload);
  }
